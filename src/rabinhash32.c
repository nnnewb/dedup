...
int rabinhash32_func(const char A[], const int offset, const int length, int w) {
    ...
    // multiple of 4. This makes the later loop easier.
    ...
    while (s < max) {
        w = compute_w_shifted(w) ^ (A[s] << 24) ^ ((A[s + 1] & 0xFF) << 16) ^ ((A[s + 2] & 0xFF) << 8) ^
            (A[s + 3] & 0xFF);
        s += 4;
    }
...
unsigned int rabin_hash(char *str) { return rabinhash32(str, 1, strlen(str)); }
