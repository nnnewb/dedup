...
static int file_chunk_cdc(int fd_src, int fd_chunk, chunk_file_header *chunk_file_hdr) {
    ...
    while (rwsize = read(fd_src, buf + bpos, exp_rwsize)) {
        ...
        if (block_sz < (BLOCK_MIN_SZ - BLOCK_WIN_SZ)) {
            ...
            block_sz = ((block_sz + tail - head) > (BLOCK_MIN_SZ - BLOCK_WIN_SZ)) ? BLOCK_MIN_SZ - BLOCK_WIN_SZ
                                                                                  : block_sz + tail - head;
        ...
        while ((head + BLOCK_WIN_SZ) <= tail) {
            ...
            hkey = (block_sz == (BLOCK_MIN_SZ - BLOCK_WIN_SZ))
                           ? adler32_checksum(win_buf, BLOCK_WIN_SZ)
            ...
            /* avoid unnecessary computation and comparsion */
            if (block_sz == 0) {
                block_sz = ((tail - head) > (BLOCK_MIN_SZ - BLOCK_WIN_SZ)) ? BLOCK_MIN_SZ - BLOCK_WIN_SZ : tail - head;
                memcpy(block_buf, buf + head, block_sz);
                head = ((tail - head) > (BLOCK_MIN_SZ - BLOCK_WIN_SZ)) ? head + (BLOCK_MIN_SZ - BLOCK_WIN_SZ) : tail;
            }
        }
...
static int delta_block_process(hashtable *htab_md5, hashtable *htab_csum, int fd_delta, char *block_buf,
                               uint64_t *offset, uint32_t block_sz) {
    uint32_t rwsize;
    uint32_t hkey;
    unsigned char md5_checksum[16 + 1] = {0};
    unsigned char csum[10 + 1] = {0};
...
static int file_delta_fsp(hashtable *htab_md5, hashtable *htab_csum, int fd_src, int fd_delta,
...
static int file_delta_cdc(hashtable *htab_md5, hashtable *htab_csum, int fd_src, int fd_delta,
                          delta_file_header *delta_file_hdr) {
    char buf[BUF_MAX_SZ] = {0};
    char block_buf[BLOCK_MAX_SZ] = {0};
    char win_buf[BLOCK_WIN_SZ + 1] = {0};
...
static int file_delta_sbc(hashtable *htab_md5, hashtable *htab_csum, int fd_src, int fd_delta,
                          delta_file_header *delta_file_hdr) {
    char buf[BUF_MAX_SZ] = {0};
