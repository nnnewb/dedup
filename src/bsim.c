...
static void usage() {
    ...
    fprintf(stderr, "Similarity detect between FILE1 and FILE2 based on block "
                    "level.\n");
    ...
    fprintf(stderr, "LCS:\n");
    fprintf(stderr, "  LCS_NOT - do not use LCS(longest lommon subsequence) "
                    "algorithms\n");
...
_BENCODE_EXIT:
    ...
    hash_free(htab);
    if (le1.str)
        free_2d_array(le1.str);
    if (le2.str)
        free_2d_array(le2.str);
    return ret;
}
